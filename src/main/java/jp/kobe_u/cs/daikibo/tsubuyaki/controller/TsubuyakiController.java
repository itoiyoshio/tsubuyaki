...
public class TsubuyakiController {
    ...
    //つぶやきを検索
    @GetMapping("/search")
    String searchTsubuyaki(@RequestParam("word") String word, Model model) {
        model.addAttribute("word", word);   //モデル属性に検索したワードをセット
        List<Tsubuyaki> list = ts.searchTsubuyaki(word); //指定した言葉を含むつぶやきを取得
        model.addAttribute("searchList", list);   //モデル属性にリストをセット
        return "search_list"; //検索結果画面を返す
    }
}